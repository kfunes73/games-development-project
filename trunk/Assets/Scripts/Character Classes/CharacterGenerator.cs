...
public class CharacterGenerator : MonoBehaviour {
	...
	private void DisplayVitals(){	
		for(int cnt = 0; cnt< Enum.GetValues(typeof(VitalName)).Length; cnt++){
			GUI.Label(new Rect (OFFSET,statStartingPos +((cnt+8)*LINE_HEIGHT),STAT_LABEL_WIDTH,LINE_HEIGHT),((VitalName)cnt).ToString());
			GUI.Label(new Rect (OFFSET + STAT_LABEL_WIDTH,statStartingPos +((cnt+8)*LINE_HEIGHT),BASEVALUE_LABEL_WIDTH,LINE_HEIGHT), _toon.GetVital(cnt).AdjustedBaseValue.ToString());
		}
	...
	private void DisplaySkills(){
		...
		for(int cnt = 0; cnt< Enum.GetValues(typeof(SkillName)).Length; cnt++){
			...
			GUI.Label(new Rect ((OFFSET*3) + STAT_LABEL_WIDTH + 20 + BASEVALUE_LABEL_WIDTH+BUTTON_WIDTH+STAT_LABEL_WIDTH,statStartingPos +(cnt*LINE_HEIGHT),BASEVALUE_LABEL_WIDTH,LINE_HEIGHT), _toon.GetSkill(cnt).AdjustedBaseValue.ToString());
		}
	}
	...
	private void DisplayCreateLabel(){
		GUI.Label(new Rect(OFFSET + STAT_LABEL_WIDTH -20 + BASEVALUE_LABEL_WIDTH+BUTTON_WIDTH+STAT_LABEL_WIDTH,statStartingPos +(9*LINE_HEIGHT),BASEVALUE_LABEL_WIDTH + 30,LINE_HEIGHT), "Creating ....", "Button");
	}
	private void DisplayCreateButton(){
	if(	GUI.Button(new Rect(OFFSET + STAT_LABEL_WIDTH -20 + BASEVALUE_LABEL_WIDTH+BUTTON_WIDTH+STAT_LABEL_WIDTH,statStartingPos +(9*LINE_HEIGHT),BASEVALUE_LABEL_WIDTH + 30,LINE_HEIGHT), "Create"))
		{
